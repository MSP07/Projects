...
void searchSuggestedWord(struct trieNode *node, string res)
{
    ...
    for (int i = 0; i < 26; i++)
    {
        ...
        if (node->children[i] != NULL)
        {
            ...
            res.push_back(i);
            searchSuggestedWord(node->children[i], res);
            res.pop_back();
        }
    }
}
...
bool suggestedWords(struct trieNode *root, string key)
{
    ...
    for (int i = 0; i < key.length(); i++)
    {
        ...
        if (node->children[key[i] - 'a'] == NULL)
        {
            ...
            searchSuggestedWord(node, key.substr(0, i));
